  ...
  return (
    <article className="blog-post">
      <Helmet>
        ...
        <meta name="description" content="The slowest thing in our own tooling wasn't a missing feature — it was years of fixes, each defending a mistake nobody had noticed. To read one record, the client hauled the whole collection over and kept one row; around that scan had grown a retry loop, a fast-miss heuristic, and a retry cap. All of it defended a scan that never needed to happen: LastDB serves a keyed read in hundredths of a second. Replacing the scan didn't tune the workarounds — it deleted them. 20s became 0.1s. On measuring per-phase before you name a cause, and how complexity accretes to defend a wrong assumption." />
      ...
      <h2>The tower</h2>
      <p>And other code had. A full scan of a large, long-lived collection is not just slow &mdash; it&rsquo;s occasionally <em>flaky</em>, coming back momentarily empty for a row that&rsquo;s plainly there. So a retry loop had been added, to ride that out. But retrying every miss made a genuinely-absent record expensive, so a &ldquo;fast-miss&rdquo; heuristic had been added on top: if the page came back populated but without your row, stop &mdash; that&rsquo;s an authoritative no. But a brand-new collection starts empty, and the retry loop would stall on it, so a cap had been added on top of <em>that</em>, to bound the wait.</p>
      ...
      <p>So the fix was a keyed read where the scan had been. And here is the part worth keeping: the tower didn&rsquo;t need to be ported, tuned, or carefully preserved. It needed to be <em>deleted</em>. The retry loop rode out a flake the scan produced &mdash; a keyed read doesn&rsquo;t flake (we hammered it a hundred and eighty times, sequential and concurrent, and it never once missed). The fast-miss heuristic sorted &ldquo;empty page&rdquo; from &ldquo;your row fell off the page&rdquo; &mdash; a keyed read has no page. The cap protected against a stall that no longer exists. We didn&rsquo;t adjust any of it. We took it all out. Reading a card went from about twenty seconds to a tenth of one; the Brain write went from five seconds to one. The change removed more code than it added.</p>
